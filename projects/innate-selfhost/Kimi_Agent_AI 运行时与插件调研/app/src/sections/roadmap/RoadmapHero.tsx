import { useRef } from 'react';
import gsap from 'gsap';
import { useGSAP } from '@gsap/react';

export default function RoadmapHero() {
  const containerRef = useRef<HTMLDivElement>(null);
  const headlineRef = useRef<HTMLHeadingElement>(null);
  const subtitleRef = useRef<HTMLParagraphElement>(null);
  const legendRef = useRef<HTMLDivElement>(null);

  useGSAP(() => {
    if (!headlineRef.current || !subtitleRef.current || !legendRef.current) return;

    const words = headlineRef.current.querySelectorAll('.word');
    gsap.fromTo(
      words,
      { y: 40, opacity: 0 },
      { y: 0, opacity: 1, stagger: 0.12, duration: 0.8, ease: 'power3.out' }
    );

    gsap.fromTo(
      subtitleRef.current,
      { y: 30, opacity: 0 },
      { y: 0, opacity: 1, duration: 0.7, delay: 0.3, ease: 'power3.out' }
    );

    gsap.fromTo(
      legendRef.current,
      { y: 20, opacity: 0 },
      { y: 0, opacity: 1, duration: 0.6, delay: 0.5, ease: 'power3.out' }
    );
  }, { scope: containerRef });

  return (
    <section
      ref={containerRef}
      className="relative min-h-[60vh] flex items-center justify-center pt-16 overflow-hidden"
    >
      {/* Background glow */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background:
            'radial-gradient(ellipse at 50% 50%, rgba(139, 92, 246, 0.08) 0%, transparent 60%)',
        }}
      />

      <div className="relative z-10 text-center px-6 max-w-[720px] mx-auto">
        {/* Breadcrumb */}
        <nav className="mb-6">
          <span className="font-['Inter'] text-[0.75rem] font-medium text-[#5A5570] tracking-wide uppercase">
            Home / Roadmap
          </span>
        </nav>

        {/* Headline */}
        <h1
          ref={headlineRef}
          className="font-['Space_Grotesk'] font-bold text-[clamp(2rem,5vw,4.5rem)] leading-[1.0] tracking-[-0.03em] text-[#E0DDF0] mb-6"
        >
          <span className="word inline-block">Roadmap</span>
        </h1>

        {/* Subtitle */}
        <p
          ref={subtitleRef}
          className="font-['Inter'] text-[1.125rem] text-[#8A85A0] leading-[1.6] max-w-[640px] mx-auto mb-10"
        >
          Our journey to democratize AI product building. Track our progress, see what&apos;s
          coming, and find where you can contribute.
        </p>

        {/* Legend */}
        <div ref={legendRef} className="flex flex-wrap items-center justify-center gap-5">
          <LegendItem color="#10B981" label="Completed" />
          <LegendItem color="#F59E0B" label="In Progress" />
          <LegendItem color="#5A5570" label="Planned" />
          <LegendItem color="#8B5CF6" label="Future" dashed />
        </div>
      </div>
    </section>
  );
}

function LegendItem({
  color,
  label,
  dashed,
}: {
  color: string;
  label: string;
  dashed?: boolean;
}) {
  return (
    <div className="flex items-center gap-2">
      <div
        className="w-2.5 h-2.5 rounded-full"
        style={{
          backgroundColor: color,
          border: dashed ? `2px dashed ${color}` : 'none',
          background: dashed ? 'transparent' : color,
        }}
      />
      <span className="font-['Inter'] text-[0.8rem] text-[#8A85A0]">{label}</span>
    </div>
  );
}
