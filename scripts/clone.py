#!/usr/bin/env python3
"""读取 registry.yaml / registry.json，按声明的 path 批量 clone / 更新项目。"""
...
def update_repo(target: Path, repo: str) -> str:
    ...
    if remote.returncode == 0:
        ...
        if current.rstrip("/").removesuffix(".git") != repo.rstrip("/").removesuffix(".git"):
            print(f"[WARN] {target.relative_to(ROOT_DIR)} origin 为 {current}，registry 为 {repo}")
    else:
        run_git(["remote", "add", "origin", repo], target)

    fetch = run_git(["fetch", "--prune", "origin"], target)
    if fetch.returncode != 0:
        print(f"[ERROR] fetch 失败: {target.relative_to(ROOT_DIR)}")
    ...
    if upstream.returncode == 0:
        ...
        for candidate in ("origin/main", "origin/master"):
            ...
            print(f"[WARN] {target.relative_to(ROOT_DIR)} 无可用远程分支，跳过更新")
            return "skipped"
    ...
    if pull.returncode != 0:
        ...
        print(f"[WARN] {target.relative_to(ROOT_DIR)} 无法 fast-forward 到 {target_ref}")
    ...
    if before == after:
        print(f"[OK]    {target.relative_to(ROOT_DIR)} 已是最新")
        return "ok"
...
def main() -> None:
    ...
    for p in projects:
        ...
        if is_git_repo(target):
            ...
            print(f"[SKIP] {rel_path} 已存在但不是 git 仓库")
            skipped += 1
        else:
            ...
            if result.returncode != 0:
                print(f"[ERROR] clone 失败: {rel_path}")
                errors += 1
            else:
                cloned += 1

    print(
        f"\n==> 完成: 克隆 {cloned} 个, 更新 {updated} 个, "
        f"跳过/已最新 {skipped} 个, 失败 {errors} 个"
    )
    if errors:
        sys.exit(1)
