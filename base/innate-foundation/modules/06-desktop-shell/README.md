...
**先可用的一层**：多个 app 共享 Cargo 编译缓存（`CARGO_TARGET_DIR`），见 [`base/innate-backend/innate-go/desktop/`](../../../desktop-cargo/)。这与后续 shell **源码 lib**（T01–T04）互补，不互相替代。

## 代码落点

- 共享 cargo build：[`base/innate-backend/innate-go/desktop/`](../../../desktop-cargo/)
...
| 任务 | 说明 | 优先级 |
|------|------|--------|
| [T00-shared-cargo-target.md](tasks/T00-shared-cargo-target.md) | 共享 `CARGO_TARGET_DIR`（已落地 `base/innate-backend/innate-go/desktop`） | P0 |
